...
!           Checking WD characteristics calculation
  ...
  IF(((X(VGRID)/R_wd).GT.(0.01)).AND.((X(VGRID)/R_wd).LT.(2))) THEN
    ...
    CALL SHOCK_SHOOT(X(VGRID), h_star, ITER, .005, func, func_prime, VGRID, RHO, P, TK, X, NELEC,SOLN)
  ENDIF  
!
...
SUBROUTINE SHOCK_SHOOT(h, hstar, max_iter, tol, func, func_prime, VGRID,RHO,P,TK,X,NELEC,SOLN)
  REAL h, hstar, tol

...
*          istart, iend  -  dummy integer indexes
      ...
      integer nlelt(3:nelemp), ielem, istart, iend
...
c
      ...
      real,allocatable,save :: elfe(:), xlinefe(:)
      integer nfe
      integer ifreq

      save nlelt, xline, nfe

      data nlelt / 15, 15, 15, 20, 22, 48, 70, 71, 157, 133 /

      if (.not.allocated(elfe)) then
         allocate(elfe(nmaxfe), xlinefe(nmaxfe))
      endif

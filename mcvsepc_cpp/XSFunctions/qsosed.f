...
c changed to a simple else to suppress compiler warning. note that rep is
c set to 1.0 earlier in the routine so this if test is unnecessary
c elseif(rep.ge.0.0d0)then
               else
                  trepdis=t
               endif
               theta0=asin(Ht/r)                           

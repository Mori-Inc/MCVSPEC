...
c
      IF ( init.EQ.0 ) THEN
         init = 1
         datdir = fgmodf()
         filion = TRIM(datdir)//'mekal1.dat'
         filrec = TRIM(datdir)//'mekal2.dat'
         filkar = TRIM(datdir)//'mekal3.dat'
         filcon = TRIM(datdir)//'mekal4.dat'
         fillin = TRIM(datdir)//'mekal5.dat'
         filcaf = TRIM(datdir)//'mekal6.dat'
...
c	close(unit=80)
 ...
 50      CONTINUE
         ...
         IF ( ierr .NE. 0 ) THEN
            wrtstr = 'Failed to get data from '//TRIM(filion)
...
c	close (unit=80)
 ...
 100     CONTINUE
         ...
         IF ( ierr .NE. 0 ) THEN
            wrtstr = 'Failed to get data from '//TRIM(filrec)
            CALL xwrite(wrtstr, 10)
            RETURN
         ENDIF
...
c	close (unit=80,status='keep')
 ...
 150     CONTINUE
         ...
         IF ( ierr .NE. 0 ) THEN
            wrtstr = 'Failed to get data from '//TRIM(filkar)
            CALL xwrite(wrtstr, 10)
            RETURN
         ENDIF
...
c	close (unit=80,status='keep')
 ...
 200     CONTINUE
         ...
         IF ( ierr .NE. 0 ) THEN
            wrtstr = 'Failed to get data from '//TRIM(filcon)
            CALL xwrite(wrtstr, 10)
            RETURN
         ENDIF
...
c        close(unit=80,status='keep')
 ...
 250     CONTINUE
         ...
         IF ( ierr .NE. 0 ) THEN
            wrtstr = 'Failed to get data from '//TRIM(fillin)
            CALL xwrite(wrtstr, 10)
            RETURN
         ENDIF
...
c        close(unit=80,status='keep')
 ...
 300     CONTINUE
         ...
         IF ( ierr .NE. 0 ) THEN
            wrtstr = 'Failed to get data from '//TRIM(filcaf)
            CALL xwrite(wrtstr, 10)
            RETURN
         ENDIF
c
...
**==mekal4.spg  processed by SPAG 4.50J  at 14:35 on 16 May 1995
      ...
      IMPLICIT NONE
C*** Start of declarations inserted by SPAG
      REAL a31 , a32 , bf , CMAX , dgfbk , dx , E1 , E2 , e2p , ej , 

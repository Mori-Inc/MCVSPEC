...
c Local variables
      ...
      DOUBLE PRECISION,ALLOCATABLE :: scat(:,:)
      DOUBLE PRECISION,ALLOCATABLE :: ntscat(:,:)
      DOUBLE PRECISION,ALLOCATABLE :: pin(:), pout(:)
      DOUBLE PRECISION,ALLOCATABLE :: pin0(:), pout0(:)
      DOUBLE PRECISION thresh, dmax, erate, odmax, rate, tau, temp

      INTEGER,ALLOCATABLE :: iscat(:,:)

      INTEGER i, idmax, ii, index, j, k

      data thresh /0.1/

      ALLOCATE(scat(NPHENERG,NPHENERG))
      ALLOCATE(ntscat(NPHENERG,NPHENERG))
      ALLOCATE(pin(NPHENERG),pout(NPHENERG))
      ALLOCATE(pin0(NPHENERG),pout0(NPHENERG))
      ALLOCATE(iscat(NPHENERG,2))
...
c  satisfy the convergence criterion
 ...
 10         continue
      ...
      DEALLOCATE(scat, ntscat, pin, pout, pin0, pout0, iscat)
...
c=====================
      ...
      INTEGER ilun
      INTEGER blocksize, rwmode, status
...
c get free unit number from xspec
      ...
      CALL xwrite('ltcz',15)
      FileName = TRIM(datadir)//'ltcz.fits'
      status = 0
      call ftopen(ilun, FileName, rwmode,  blocksize, status)

      IF ( status .NE. 0 ) THEN
         ctmp = 'Failed to open '//TRIM(Filename)
         CALL xwrite(ctmp,10)
         RETURN
      ENDIF
c clav
...
c rldisp
      ...
      CALL xwrite('comp',15)
      FileName = TRIM(datadir)//'comp.fits'
      call ftopen(ilun, FileName, rwmode,  blocksize, status)
c ecomp
...
c igcomp
      ...
      FileName = TRIM(datadir)//'azntc.fits'
      call ftopen(ilun, FileName, rwmode,  blocksize, status)
...
c compr(NLORGAM,NPHENERG)
      ...
      CALL xwrite('clt2',15)
      FileName = TRIM(datadir)//'clt2.fits'
      call ftopen(ilun, FileName, rwmode,  blocksize, status)
c cltr(141,241)
...
c cltd(141,241)
      ...
      FileName = TRIM(datadir)//'azcomp.fits'
      call ftopen(ilun, FileName, rwmode,  blocksize, status)
c compr(NLORGAM,NPHENERG)
...
c ctab(520483)
      ...
      FileName = TRIM(datadir)//'azecomp.fits'
      call ftopen(ilun, filename, rwmode,  blocksize, status)
...
c ectab(218997)
      ...
      CALL xwrite('azpp',15)
      FileName = TRIM(datadir)//'azpp.fits'
      call ftopen(ilun, FileName, rwmode,  blocksize, status)
...
c ppf(NLORGAM,NPHENERG)
      ...
      CALL xwrite('thrm',15)
      FileName = TRIM(datadir)//'thrm.fits'
      call ftopen(ilun, FileName, rwmode,  blocksize, status)
c thann(3,2)
...
c cr(162,2)
      ...
      CALL xwrite('htc',15)
      FileName = TRIM(datadir)//'htc.fits'
      call ftopen(ilun, FileName, rwmode,  blocksize, status)
c htcr(161,4)
...
c htcdl(NLORGAM,4)
      ...
      CALL xwrite('brem',15)
      FileName = TRIM(datadir)//'brem.fits'
      call ftopen(ilun, FileName, rwmode,  blocksize, status)
c brem(61)
...
c      print *,'thresh?,dmax',thresh,dmax
      ...
      DEALLOCATE(scat, pin, pout, pin0, pout0, iscat)

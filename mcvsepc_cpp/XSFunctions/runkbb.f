     ...
     &                  FCOL, RFLAG, LFLAG, PHOTAR, FLUXE, NEX1, NEX2)

      IMPLICIT NONE

      Integer NE
      Real ETA, ASTAR, THETA, MBH, MDD, DBH, FCOL
...
C     observed photon energy between EAR(i) and EAR(i+1).
      ...
      Real,allocatable,save :: FLUX0(:,:,:,:)
      Real FLUX(NENER), E(NENER), GI(NG) 
     ...
     $       (rflagsav .ne. rflag)) then
         datdir = fgmodf()
         filenm = TRIM(datdir)//'kerrbb.fits'
...
C Read four sets of grid values
         ...
         contxt = 'Failed to read GRIDVALS data from '//TRIM(filenm)
         IF ( status .NE. 0 ) GOTO 999
...
C Read the appropriate flux data
         ...
         if(.not.allocated(flux0)) allocate(FLUX0(NS,NTH,NG,NENER))
         CALL ftgcve(ilun, icol, 1, 1, ng*ns*nth*nener, 0.0, 
     &               flux0, qanyf, status)
         contxt = 'Failed to read FLUX data from '//TRIM(filenm)
         IF ( status .NE. 0 ) GOTO 999

...
c     Assumes thetaobs is in radians.
      ...
      double precision dvobs(nv),KNindex(nv),UsyncKN,UexKN
...
c     Do Broad Line Region Spectrum in jet frame
      ...
      if (zdiss.lt.RBLR) then
         UBLR = (17.0*BLF**2.0/12.0)*f_BLR*Ldisc
     &        /(4.0*pi*c*(RBLR*Rg)**2.0)
         mu1 = (1.0+(RBLR/zdiss)**2.0)**(-1.0/2.0)
         b = BLF 
c write(*,*) mu1,beta,BLF,b
      else if ((RBLR.le.zdiss).and.(zdiss.le.3.0*RBLR)) then
         write(*,*) '! zdiss > R_BLR !'
         U1 = (17.0*BLF**2/12.0)*f_BLR*Ldisc/(4.0*pi*c*(RBLR*Rg)**2)
         mu1 = (1.0+(1.0/3.0)**2)**(-1.0/2.0)
         mu2 = (1.0-(1.0/3.0)**2)**(1.0/2.0)
         U2 = f_BLR*Ldisc/(4.0*pi*c*(RBLR*Rg)**2)*BLF**2/(3.0*beta)*
     &        (2.0*(1.0-beta*mu1)**3-(1.0-beta*mu2)**3-(1.0-beta)**3)
         ...
         write(*,*) '! zdiss > 3R_BLR !'
         mu1 = (1.0+(RBLR/zdiss)**2)**(-1.0/2.0)
         mu2 = (1.0-(RBLR/zdiss)**2)**(1.0/2.0)
         UBLR = f_BLR*Ldisc/(4.0*pi*c*(RBLR*Rg)**2)*BLF**2.0/(3.0*beta)*
     &        (2.0*(1.0-beta*mu1)**3-(1.0-beta*mu2)**3-(1.0-beta)**3)
         b = BLF*(1.0-beta*mu1) 
      end if
      if (RBLR.ne.0.0) then
...
c     Do Torus Spectrum in jet frame
      ...
      else if ((RIR.le.zdiss).and.(zdiss.le.3.0*RIR)) then
         write(*,*) '! zdiss > R_IR !'

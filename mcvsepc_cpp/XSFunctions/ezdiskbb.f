...
c                  array is not calculated by this program.
      ...
      Integer MAXIN
      parameter (MAXIN=24999)
      Integer,allocatable :: ILO(:), IHI(:)
      Integer,save :: IREAD = 0
      Integer I, J, M, ilun, ierr, index

      Real,allocatable :: FIN(:), FNEW(:), ENEW(:)
      Real,allocatable,save :: FLUX(:), E(:)
      Real SLOPE, F1, F2, F, Z
...
c       so that the data file will not be read again. 
      ...
      ALLOCATE(ILO(0:MAXIN+1))
      ALLOCATE(IHI(0:MAXIN+1))
      ALLOCATE(FIN(0:MAXIN+1))
      ALLOCATE(ENEW(0:MAXIN),FNEW(0:MAXIN))
...
c this model does not calculate errors
      ...
      if (iread .ne. 99) then
         ...
         allocate(FLUX(0:MAXIN), E(0:MAXIN))
         datdir = fgmodf()
         filenm = TRIM(datdir)//'ezdiskbbdata.fits'
         ...
         call ftgcve(ilun, 1, 1, 1, MAXIN+1, " ", e, qanyf, ierr)
         call ftgcve(ilun, 2, 1, 1, MAXIN+1, " ", flux, qanyf, ierr)
         call ftclos(ilun, ierr)
         CALL frelun(ilun)

         if ( ierr .NE. 0 ) then
            contxt = 'EZDISKBB: Failed to read '//
     &           filenm(:MAX(LEN_TRIM(filenm),225))
...
c       in units of photons/cm^2/s/bin.
 ...
 70   enddo
      ...
      DEALLOCATE(ILO,IHI,FIN,ENEW,FNEW)

...
c     this is number of bins in my tables (between ~0.1 and 200 keV)
     ...
     $     eart(ne2), phottmp(ne2), enel
      ...
      INTEGER ilun, ierr, iilow, iiup, nshag, nkount
...
c     this is normalization of the incident power law for the first point
      ...
      ALLOCATE(spectr(nr,nmaxp),sptemp(4,nr,nmaxp))
...
c     and picks the grid
      ...
      if (icall.eq.0) then
         slovo = fgmodf()
         slovo = TRIM(slovo)//'xiondata.fits'
         CALL getlun(ilun)
         call ftopen(ilun, slovo, 0, block, ierr)
         IF ( ierr .NE. 0 ) THEN
            contxt = 'XION_REF: Failed to open '//TRIM(slovo)
...
c      parameter (ntest = 1441, ncmax = 16)      
     ...
     5     weight(ncmax), afe(3)
      ...
      INTEGER ilun, ierr, jh, m, icall, i, np, ntmax
      ...
      SAVE abund, gam1, ecut1, fxd1, tauex, iang, grav, ntmax
...
c         mu_v(m) = (m-0.5)/(float(mumax_v))
      ...
      i = 1
      filenm = fgmodf()
      filenm = TRIM(filenm)//'xiondata.fits'      
      CALL getlun(ilun)
      ierr = 0

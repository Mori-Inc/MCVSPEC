...
c-----------------------------------------------------------------------------
      ...
      DOUBLE PRECISION,ALLOCATABLE,SAVE :: flux(:,:)
      DOUBLE PRECISION fluxin(21)

      INTEGER ilun, ios, ninp, minp
      ...
      SAVE ninp, minp, temp, ene, t1, t2, magsve
...
c directory.
      ...
      pname = 'NSA_FILE'
      filenm = fgmstr(pname)
      IF ( LEN_TRIM(filenm) .EQ. 0 ) THEN
         filenm = fgmodf()
         filenm = TRIM(filenm)//'nsadata.fits'
      ENDIF
     ...
     &        'The magnetic field must be one of 0, 1e12, or 1e13 G', 5)
      ...
      contxt = 'Using '//TRIM(filenm)
      CALL xwrite(contxt, 25)

      ios = 0
      CALL FTOPEN(ilun,filenm,0,index,ios)
      IF ( ios .NE. 0 ) THEN
         contxt = 'NSA: Failed to open '//TRIM(filenm)
...
1     continue
      ...
      do i=0,ne
         ...
         if(e.le.ene(ninp)) then
            ...
            f1=REAL(flux(jt-1,k-1))
     &           +de*(REAL(flux(jt-1,k)-flux(jt-1,k-1)))

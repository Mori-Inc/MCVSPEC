...
c     later passes.
     ...
     &     countsl(nfreq)
      ...
      INTEGER l, i, j, k, iT, ig, ic, ios, ir
      ...
      save Tlog,gravlog,critmu,EkeV,nTemp,ngrav,nmu,nf,nread
...
c Open and read nsatmosdata.fits the first time round.  The information is saved
      ...
      if (nread.ne.99) then
         ...
         IF ( LEN_TRIM(filenm) .EQ. 0 ) THEN
            datdir = fgmodf()
            filenm = TRIM(datdir)//'nsatmosdata.fits'
         ENDIF
         ...
         IF ( ios .NE. 0 ) THEN
            outstr = 'NSATMOS : failed to open '//TRIM(filenm)
            CALL xwrite(outstr, 10)
            RETURN
         ENDIF
         outstr = 'Opened and reading '//TRIM(filenm)
         CALL xwrite(outstr, 10)
...
c     interpolation.
         ...
         allocate(fluxl(nparT,nparg,nparm,nfreq))
         ir = 1
         do i=1,nTemp
            do j=1,ngrav
               do k=1,nmu

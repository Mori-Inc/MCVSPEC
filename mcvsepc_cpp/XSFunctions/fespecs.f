...
*
      ...
      allocate(elfe(nmaxfe),xlinefe(nmaxfe),index(nmaxfe))
...
c read the outfe.fits file - each ionization stage is in its own extension
      ...
      filenm = TRIM(pathd) // 'outfe.fits'
      CALL getlun(lun)
      CALL ftopen(lun, filenm, 0, iblock, ierr)
      contxt = 'Failed to open '// TRIM(filenm)
     ...
     &               qanyf, ierr)
         ...
         contxt = 'Failed to read data from '//TRIM(filenm)
         IF ( ierr .NE. 0 ) GOTO 999
...
c now repeat for the ferad.fits file
      ...
      DO ion = 1, 8
         ...
         CALL ftmrhd(lun, 1, hdutyp, ierr)
         contxt = 'Failed to move to next HDU in '//TRIM(filenm)
         IF ( ierr .NE. 0 ) GOTO 999
...
cc      close(68)
cc      open(69,file=TRIM(pathd) // 'outbefem',status='old')
cc      nl(nionmx-1) = 302
cc      nbl=nl(nionmx)
cc      read(69,915) ( il,jl,k(j),eij(j),c(j),
cc     . p1m(j),p2m(j),p3m(j),p4m(j),p5m(j),j = 2+nbl,nl(nionmx-1)+nbl )
cc      close(69)
cc      open(70,file=TRIM(pathd) // 'befexxiii.rad', status='old')
...
*                
     ...
     .  p1m(nlmx),p2m(nlmx),p3m(nlmx),p4m(nlmx),p5m(nlmx)
      ...
      common /comtr/ ntr,ntrpnti,ntrpntj,eln,einsta
      dimension eline(*), flx(*)
      real, allocatable :: br(:), rates(:)

      allocate(br(ntrmxl),rates(ntrmxl))

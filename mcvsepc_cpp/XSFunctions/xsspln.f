...
c  that section
            ...
            if (el .le. p .AND. eh .gt. p) then

               integval = ((d*eh+c*eh**2/2+b*eh**3/3+a*eh**4/4) -
     &                     (d*p+c*p**2/2+b*p**3/3+a*p**4/4))

c              the math here ensures that fractionally-covered bins are 
c              handled right in the case where another spline covers
c              an adjacent energy range
               photar(i) = SNGL((integval/(eh-p))**((eh-p)/(eh-el)))

c  if energy range lies completely in valid interval then average over entire
c  energy range

            else if (el .ge. p .AND. eh .le. q) then
...
c  that section
            ...
            else if (eh .ge. q .AND. el .lt. q) then

               integval = ((d*q+c*q**2/2+b*q**3/3+a*q**4/4) -
     &                     (d*el+c*el**2/2+b*el**3/3+a*el**4/4))
               photar(i) = SNGL((integval/(q-el))**((q-el)/(eh-el)))

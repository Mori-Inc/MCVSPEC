...
const std::map<string,Real>& FunctionUtility::getAllXFLT(int ifl)
{
  std::map<int,std::map<string,Real> >::const_iterator itMap = 
     s_XFLT.find(ifl);
  if (itMap == s_XFLT.end())
  {
     // Throw silently, let calling function decide how to handle.
     throw YellowAlert();
  }
  return itMap->second;
}
...
void FunctionUtility::tableInterpolate(const RealArray& energyArray,
				       ...
				       const bool readFull,
				       const std::map<string,Real>& spectrumXFLT)
{
  using namespace std;
  ...
  static map<string,heasp::table> saveTables;
  
  // check whether we already have the table from the input filename

  if ( saveTables.find(fileName) == saveTables.end() ) {
    heasp::table readTable;
    int status = readTable.read(fileName, readFull);
    if ( status != 0 ) {
      FunctionUtility::xsWrite("Failed to read "+fileName, 5);
      return;
    }
    saveTables[fileName] = readTable;
  }

  heasp::table& inTable = saveTables[fileName];
  ...
  int status = inTable.getValues(params, spectrumXFLT, minEnergy, maxEnergy, 
				 tableEnergyBins, tableValues, tableErrors);
  if ( status != 0 ) {
    FunctionUtility::xsWrite(heasp::SPgetErrorStack(), 5);
    heasp::SPclearErrorStack();
    return;
  }

  // get the values to be used for energies below or above those tabulated in the
  // table. if the relevant keywords were not in the table FITS file these limits
  // have been set to -1 so use 0 for add or exp table models and 1 for mul table models.

  Real loLimit = inTable.getLowEnergyLimit();
  Real hiLimit = inTable.getHighEnergyLimit();
  if ( loLimit == -1.0 ) {
    if ( tableType == "add" || tableType == "exp" ) loLimit = 0.0;
    if ( tableType == "mul" ) loLimit = 1.0;
  }
  if ( hiLimit == -1.0 ) {
    if ( tableType == "add" || tableType == "exp" ) hiLimit = 0.0;
    if ( tableType == "mul" ) hiLimit = 1.0;
  }
  ...
  findFirstBins(tableEnergyBins, energyArray, heasp::FUZZY, inputBin, outputBin);
  initializeBins(tableEnergyBins, energyArray, heasp::FUZZY, inputBin, outputBin,
...
// version of tableInterpolate which takes the spectrum number as input instead of
// the map of XFLT names and values.
				       ...
				       const bool readFull)
{
  try
  {
     const std::map<string,Real>& xfltMap = getAllXFLT(spectrumNumber);
     tableInterpolate(energyArray, params, fileName, fluxArray, fluxErrArray,
		   initString, tableType, readFull, xfltMap);
  }
  catch (YellowAlert&)
  {
     tableInterpolate(energyArray, params, fileName, fluxArray, fluxErrArray,
		   initString, tableType, readFull);
  }
  return;
}

int FunctionUtility::tableInfo(const string fileName, int& numberParams, int& numberSpectra,
				int& numberEnergies, bool& isAdditive, bool& isRedshift,
				bool& isEscale)
{
  using namespace std;
  ...
  if ( saveTables.find(fileName) == saveTables.end() ) {
    heasp::table readTable;
    int status = readTable.read(fileName, false);
  ...
  numberParams = inTable.getNumberParameters();
  numberSpectra = inTable.getNumberSpectra();
  numberEnergies = inTable.getNumberEnergies();
  isAdditive = inTable.getisAdditive();
  isRedshift = inTable.getisRedshift();
  isEscale = inTable.getisEscale();

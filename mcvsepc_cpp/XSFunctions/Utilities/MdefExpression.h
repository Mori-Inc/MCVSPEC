...
class MdefExpression : public AbstractExpression  //## Inherits: <unnamed>%47F12AF502CF
{
  ...
  private:
    ...
    typedef enum {ENG, ENGC, NUM, PARAM, OPER, UFUNC, BFUNC, LPAREN, RPAREN, 
		  COMMA, XSMODEL, CONXSMODEL, TABLEMODEL, MDEF} ElementType;

  public:
      MdefExpression(const MdefExpression &right);
      MdefExpression (std::pair<Real,Real> eLimits, const string& compType, const string& mdefName);
      ...
      void evaluate (const RealArray& energies, const RealArray& parameters, int spectrumNumber,
		     RealArray& flux, RealArray& fluxErr, const string& initString) const;
      ...
      const std::set<string>& usingOtherMdefs() const;
      const string& mdefName() const;
      const bool& callsSpecDependentFunctions() const;
  ...
  private:
      ...
      //	binary and xspec model function calls.
      void verifyFuncCommas (size_t* idxElem, size_t* ixsFunc, 
                const std::vector<size_t>& xsModCommas) const;
      void convolveEvaluate (const RealArray& energies, const RealArray& parameters, int spectrumNumber,
			     RealArray& flux, RealArray& fluxErr, const string& initString) const;
      // special case of an mdefine convolution model which consists only of an xspec convolution model
      void singleConvolveEvaluate (const RealArray& energies, const RealArray& parameters, int spectrumNumber,
			     RealArray& flux, RealArray& fluxErr, const string& initString) const;
      // test whether singleConvolveEvaluate can be used
      bool isSingleConvolve() const;
  ...
  private: //## implementation
    // Data Members for Class Attributes
      ...
      Real m_eHigh;
      string m_compType;
      // Store names of any mdef components that this component
      //  makes use of.  Order is unimportant.  This is useful
      //  for determining dependencies upon deletion of an mdef
      //  component.
      std::set<string> m_usingOtherMdefs;
      string m_mdefName;
      // This boolean is set true if the expression calls a standard xspec
      // function that is spectrum dependent
      bool m_callsSpecDependentFunctions;
...
inline const string& MdefExpression::componentType() const
{
  return m_compType;
}

inline const std::set<string>& MdefExpression::usingOtherMdefs() const
{
   return m_usingOtherMdefs;
}

inline const string& MdefExpression::mdefName() const
{
   return m_mdefName;
}

inline const bool& MdefExpression::callsSpecDependentFunctions() const
{
   return m_callsSpecDependentFunctions;
}
